 ...
 */
package com.eln.lib.ui.slidemenu;
...
import com.eln.lib.ui.slidemenu.ScrollDetectors.ScrollDetector;
...
public class SlideMenu extends ViewGroup {
	...
	private final static int POSITION_X =1;
	private final static int POSITION_Y =2;
	...
	private int mPosition_Orientation =POSITION_M;
	...
	private static boolean mIsOpenSlideMenu = false;
	...
	public static boolean getSlideMenuOpen(){
		return mIsOpenSlideMenu;
	}
	 ...
	 * 
	 */
	private void resetEnterFocusActTime() {
		int nStartEnterToFocusActTime = (int)System.currentTimeMillis();
	}
	...
	public boolean isOpen() {
		mIsOpenSlideMenu = false;
		if ((STATE_OPEN_MASK & mCurrentState) != 0) {
			mIsOpenSlideMenu = true;
			resetEnterFocusActTime();
		}else{
			mIsOpenSlideMenu = false;
		}
 ...
 * @return
 */
	private boolean isScrollX(float disX,float disY){
		if(mPosition_Orientation==POSITION_M){
			if(Math.abs(disX)>Math.abs(disY) && Math.abs(disX)>=mTouchSlop){
				mPosition_Orientation = POSITION_X;
			}else if(Math.abs(disY)>=mTouchSlop){
				mPosition_Orientation = POSITION_Y;
			}else{
				mPosition_Orientation=POSITION_M;
			}
		}
		if(mPosition_Orientation == POSITION_X){
			return true;
		}else{
			return false;
		}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			mPressedY = mLastMontionY = y;
			mPosition_Orientation=POSITION_M;
			mIsTapContent = isTapContent(x, y);
			return isOpen() && mIsTapContent;
		case MotionEvent.ACTION_MOVE:
			float distanceX = x - mPressedX;
			float distanceY = y - mPressedY;
			if (isScrollX(distanceX,distanceY) && mIsTapContent && mPosition_Orientation==POSITION_X) {
				if (!canScroll(this, (int) distanceX, (int) x, (int) y)) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			float disX = x - mPressedX;
			float disY = y - mPressedY;
			boolean bn=isScrollX(disX,disY);
			if (bn && isTapContent
					&& currentState != STATE_DRAG && mPosition_Orientation==POSITION_X) {
			...
			if(bn && mPosition_Orientation==POSITION_X){
				drag(mLastMotionX, x);
			}
			mLastMotionX = x;
			mLastMontionY =y;
