...
public class FixedTabsView extends LinearLayout implements ViewPager.OnPageChangeListener {
	...
	private int mDividerColor = 0x020853;
	private int mDividerMarginTop = 12;
	private int mDividerMarginBottom = 10;
...
//		setBackgroundResource(R.drawable.common_menu_bar_bg);
		removeAllViews();
		mTabs.clear();
		
		if (mAdapter == null) return;
		mTabsCount = mAdapter.getCount();
		for (int i = 0; i < mAdapter.getCount(); i++) {			
			...
			tab.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					int radio=mPager.getCurrentItem()/mTabsCount;
					int mIndex=radio*mTabsCount+index;
		...
		selectTab(mPager.getCurrentItem()%mTabsCount);
	}
	...
	private void selectTab(int position) {
		...
		for (int i = 0, pos = 0; i < getChildCount(); i++) {
			...
			if (this.getChildAt(i) instanceof FixedTabButton) {
